...
// Check if live preview is disabled
const isLivePreviewDisabled = typeof process !== 'undefined' && process.env.DISABLE_LIVE_PREVIEW === "true";
...
const LivePreviewComponent = React.memo(LivePreviewBase, (prev, next) => {
  return String(prev.children) === String(next.children);
});

// Export a conditional component that returns null if disabled
export const LivePreview = (props: PlaygroundProps) => {
  if (isLivePreviewDisabled) {
    return (
      <div className="refine-live-preview mb-6">
        <BrowserWindow url="http://localhost:3000" hasBottom={!props.previewOnly}>
          <div
            className={clsx(styles.playgroundPreview, "live-editor-wrapper")}
            style={{
              maxHeight: props.previewHeight,
              minHeight: props.previewHeight,
              overflow: props.disableScroll ? "hidden" : undefined,
              position: "relative",
              display: "flex",
              alignItems: "center",
              justifyContent: "center",
        ...
        {!props.previewOnly && <Editor hidden={props.hideCode} code={String(props.children)} />}
      </div>
    );
  }
  
  return <LivePreviewComponent {...props} />;
};
